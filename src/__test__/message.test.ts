...
import { Message, MessageBuilder } from "../wechat/message.js";
import { MessageType, MessageState, MessageItemType } from "../wechat/types.js";
...
describe("MessageBuilder", () => {
  it("should build message with content", () => {
    const msg = new MessageBuilder().withContent("Hello").build();
  ...
  it("should not add item_list when content is empty", () => {
    const msg = new MessageBuilder().withContent("").build();
  ...
  it("should build message with custom item_list", () => {
    ...
    const msg = new MessageBuilder().withItemList(items).build();
    expect(msg.item_list).toEqual(items);
  });
  ...
  it("should allow chaining all methods", () => {
    ...
    expect(msg.message_state).toBe(MessageState.FINISH);
    expect(msg.item_list).toHaveLength(1);
    expect(msg.item_list?.[0].text_item?.text).toBe("Test message");
  });

  it("should override item_list when withItemList is called after withContent", () => {
    const customItems = [{ type: MessageItemType.IMAGE }];
    const msg = new MessageBuilder()
      .withContent("Will be overridden")
      .withItemList(customItems)
      .build();
    expect(msg.item_list).toEqual(customItems);
  });
});
